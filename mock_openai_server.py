...
def build(stage, user_msg):
    ...
    if stage == "summarise":
        sentiment = ("Angry" if re.search(ANGRY, low)
                     else "Frustrated" if re.search(FRUSTRATED, low)
                     else "Positive" if re.search(POSITIVE, low)
                     else "Polite" if "thank" in low or "please" in low
                     else "Neutral")
        escal = "High" if re.search(ESCALATION, low) else ("Medium" if sentiment == "Angry" else "Low")
        missing = []
        if order_id == "NOT_PROVIDED":
            missing.append("order ID")
        if vague:
            missing += ["product name", "description of the problem"]
        if " OR " in order_id:
            missing.append("unconfirmed order ID")
        return {
            "order_id": order_id,
            "issue_category": category,
            "product": "NOT_PROVIDED" if vague else "[MOCK] product extracted from ticket",
            "customer_request": "[MOCK] the customer's requested resolution",
            "summary": f"[MOCK SUMMARY] {category} on order {order_id}. "
                       f"Generated by the mock endpoint, not by an LLM.",
            "urgency": "High" if escal == "High" else ("Medium" if not vague else "Low"),
            "sentiment": sentiment,
            "escalation_risk": escal,
            "missing_info": missing,
        }

    if stage == "judge_summary":
        base = 3 if vague else 4
        scores = {
            "faithfulness": 5,
            "completeness": base,
            "conciseness": 5,
        ...
        return {**scores,
                ...
                "verdict": "PASS" if overall >= 4.0 else "REVIEW",
                "justification": "[MOCK] wiring-test score, not a real evaluation."}

    if stage == "respond":
        needs_review = bool(re.search(ESCALATION, low)) or vague
        return {
            "response_text": ("Hi there,\n\n[MOCK REPLY] This text was produced by the mock "
                              f"endpoint for wiring tests, not by an LLM. Issue: {category}, "
                              f"order {order_id}.\n\nThanks for your patience,\n"
                              "ShopNest Support Team"),
            "next_step": "[MOCK] next step",
            "policies_applied": ["P3"] if category == "Wrong Item" else ["P4"],
            "requires_human_review": needs_review,
            "review_reason": "escalation risk or unclear ticket" if needs_review else "",
        }
    ...
    if stage == "judge_response":
        scores = {
            ...
            "empathy_and_tone": 4,
            "policy_compliance": 5,
            "clarity_of_next_step": 4,
            "factual_safety": 5,
            "professionalism": 4,
        }
        overall = round(sum(scores.values()) / 6, 1)
        return {**scores,
                "overall_score": overall,
                "verdict": "PASS" if overall >= 4.0 else "REVIEW",
                "policy_violations": [],
